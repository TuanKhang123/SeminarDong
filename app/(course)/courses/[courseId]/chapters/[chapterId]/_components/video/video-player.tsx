...
import { MediaProvider, useMediaRef, useMediaSelector, useMediaDispatch, MediaActionTypes } from "media-chrome/react/media-store";
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="max-w-7xl bottom-0 translate-x-[-50%] left-[50%]" >
        <p className="text-lg">{`Add note at:`} <strong>{`${TimeCustomize}`}</strong> </p>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-5">
          ...
          <div className="flex justify-end gap-5">
            <Button type="button" variant='secondary' onClick={() => setIsDialogOpen(!isDialogOpen)}>Cancel</Button>
...
const VideoPlayer = ({
  ...
  const [timeMoving, setTimeMoving] = useState<number>(0)


  const [timeNote, setTimeNote] = useState()
  const [openListNote, setOpenListNote] = useState<boolean>(false)
